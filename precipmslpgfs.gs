...
if (H<=240)&(H>=12)
    PREVIOUSH=H-6
    if PREVIOUSH <10
        PREVIOUSFULLH = "00"%PREVIOUSH
    endif
    if (PREVIOUSH<100)&(PREVIOUSH>=10)
        PREVIOUSFULLH="0"%PREVIOUSH
    endif
endif
if H>240
    PREVIOUSFULLH=H-12
...
'open /home/mint/controlfiles/'%MODEL'/'%INIT_INTDATE''%INITHOUR'/surface_APCP_'%PREVIOUSFULLH'.ctl'
'q dims'
***** ***** Define Region ***** *****
...
'define precip = ((apcpsfc.1)-(apcpsfc.7(t=1)))*0.03937007874'
'd precip'
...
**** MAXVAL (Precip)
'd amax(((apcpsfc.1)-(apcpsfc.7(t=1)))*0.03937007874, lon='%LON1', lon='%LON2', lat='%LAT1', lat='%LAT2')'
...
**** MINVAL (Precip)
'd amin(((apcpsfc.1)-(apcpsfc.7(t=1)))*0.03937007874, lon='%LON1', lon='%LON2', lat='%LAT1', lat='%LAT2')'
